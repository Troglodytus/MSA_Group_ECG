...
for i = 1:length(window_labels)
    if ismember(window_labels(i), [0, 1, 12, 38])
...
nWindows = size(normalized_windows, 1);
seed = 40;
rng(seed);
...
trainingData = normalized_windows(cp.training, :);
trainingTargets = targetMatrix(:, cp.training)';

% Use logical indexing to select testing data
testingData = normalized_windows(cp.test, :);
testingTargets = targetMatrix(:, cp.test)';
